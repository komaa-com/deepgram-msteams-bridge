...
Use it to: confirm your bridge works and hear your Deepgram agent on a real call in minutes.
...
- **StandIn-side (tier limits):** when a sandbox/free daily cap or a subscription max-minutes governor is reached, StandIn sends an `assistant.say` message with a short goodbye line. The bridge speaks it - the exact text via standalone Aura TTS when `DEEPGRAM_TTS_MODEL` is set, otherwise the live agent speaks it verbatim via `InjectAgentMessage` - and the call is torn down by StandIn. The caller hears a clean sign-off rather than a sudden drop.
- **Bridge-side (`MAX_CALL_MINUTES`):** your own hard cap per call, useful because Deepgram knows nothing about your budget. See [Governors and Privacy](/deepgram-msteams-bridge/governors-and-privacy/).
